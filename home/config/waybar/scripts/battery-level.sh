...
BATTERY_LEVEL_ADJUSTED=$(( BATTERY_LEVEL * 100 / $(cat /sys/class/power_supply/BAT0/charge_control_end_threshold)))
...
if [ "$BATTERY_LEVEL_ADJUSTED" -eq 100 ] && [ "$BATTERY_STATE" == "fully-charged" ] && [ ! -f $FILE_FULL ]; then
  notify-send -a "state" "Battery Charged (${BATTERY_LEVEL_ADJUSTED}%)" "You might want to unplug your PC." -i "battery-full" -r 9991
  touch $FILE_FULL

# if the battery is low and is discharging
elif [ "$BATTERY_LEVEL_ADJUSTED" -le $WARNING_LEVEL ] && [ "$BATTERY_STATE" == "discharging" ] && [ ! -f $FILE_WARNING ]; then
  brightnessctl -s set 20%
  notify-send -a "state" "Battery Low (${BATTERY_LEVEL_ADJUSTED}%)" "You might want to plug in your PC." -u critical -i "battery-caution" -r 9991 -h string:fgcolor:\#fab387 -h string:frcolor:\#fab387
  touch $FILE_WARNING

# if the battery is critical and is discharging
elif [ "$BATTERY_LEVEL_ADJUSTED" -le $CRITICAL_LEVEL ] && [ "$BATTERY_STATE" == "discharging" ] && [ ! -f $FILE_CRITICAL ]; then
  brightnessctl set 10%
  notify-send -a "state" "Battery Critical (${BATTERY_LEVEL_ADJUSTED}%)" "Plug in your PC now." -u critical -i "battery-empty" -r 9991
  touch $FILE_CRITICAL
fi
